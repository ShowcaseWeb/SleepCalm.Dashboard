...
# =============================================================================
# 2. TODOS LOS TEXTOS EN ESPAÑOL (SIN BOTÓN DE IDIOMA)
# =============================================================================
...
# =============================================================================
# 3. CAPA DE DATOS (EXACTAMENTE IGUAL)
# =============================================================================
...
# =============================================================================
# 4. CAPA DE MÉTRICAS (EXACTAMENTE IGUAL - NO CAMBIA NADA)
# =============================================================================
...
def calcular_sla(df_):
    ...
    if sem_classificacao > 0:
        resultado["alerta"] = f"{sem_classificacao} entrega(s) sin clasificación en 'On Time'."
...
# =============================================================================
# 5. CAPA DE VISUALIZACIÓN (EXACTAMENTE IGUAL)
# =============================================================================
...
def fig_comparacao_transp(df_grp):
    fig = go.Figure()
    fig.add_trace(go.Bar(x=df_grp["Proveedor"], y=df_grp["NoPrazo"], name="A Tiempo", marker_color=COLOR_SUCCESS,
...
with st.sidebar:
    ...
    ano_sel = st.selectbox(TEXTO['year'], anos_disp, index=len(anos_disp) - 1)
    ...
    st.markdown("---")
    st.link_button(TEXTO['returns'], "https://script.google.com/a/macros/sleepcalm.com.br/s/AKfycbzyOsb6FzdRee9Mn88h86fPx7B7ZmZoxNZLP-brNgUZr9-BRFhW3Dt49_QeRe59Mhg6yg/exec", use_container_width=True)
...
with c4:
    ...
    avg_txt = f"{avg_d:.1f} {TEXTO['days']}" if avg_d is not None and not np.isnan(avg_d) else "--"
...
if not df_abertos.empty:
    ...
    st.caption(f"Total: {len(df_abertos)} pedidos críticos")
...
<div style="text-align:center;padding:1.5rem 0 0.5rem;color:#64748b;font-size:0.7rem;border-top:1px solid {GRID_COLOR};margin-top:1.5rem">
  {TEXTO['footer']} | Meta SLA: {META_SLA}% | {TEXTO['updated']}: {hoje.strftime('%d/%m/%Y %H:%M')}
</div>
""", unsafe_allow_html=True)
